...
import qualified Monads.Error as E
import           Phases.Parser

import           Control.Monad.Except
import           Test.HUnit

newtype TestEnvironment a = TestEnvironment {unwrap :: Either Error a}
  deriving (Functor, Applicative, Monad, MonadError Error, E.MonadError)
...
testsParser = TestList [
  ...
  mkTest "Unclosed pipe 1"                  " cmd1 |"                               $ Nothing,
  mkTest "Unclosed pipe 2"                  "cmd1 |"                                $ Nothing,
  mkTest "Unclosed pipe 3"                  " cmd1 | "                              $ Nothing,
  mkTest "Unclosed pipe 4"                  "cmd1 | "                               $ Nothing,
  mkTest "Unclosed pipe 5"                  " cmd1| "                               $ Nothing,
  mkTest "Unclosed pipe 6"                  "cmd1| "                                $ Nothing,
  mkTest "Unclosed pipe 7"                  " cmd1|"                                $ Nothing,
  mkTest "Unclosed pipe 8"                  "cmd1|"                                 $ Nothing,
