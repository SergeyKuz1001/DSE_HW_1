...
* *MonadPM* — монада для параллельного запуска и остановки процессов, оперирует типами `Stream` и `Process`, определяемыми типами-экземплярами
...
В данной командной оболочке переменные также имеют свою иерархию, а именно переменная делится на

* *стабильную* (Stable) — ту, значение которой изменяется контролируемо (следовательно, значение такой переменной можно хранить); и
* *нестабильную* (Volatile) — ту, значение которой изменяется неконтролируемо (более точно, его сложно хранить в актуальном состоянии, как, например, время, значение которого постоянно меняется).

Стабильная же переменная делится на

* *обычную* (Usual) — ту, имя которой состоит из латинских символов, цифр и знаков подчёркивания (первый символ строго латинский); и
* *специфическую* (Specific) — ту, имя которой не относится к имени обычной переменной (это, например, `?`).

Обычно всё в данной программе, что работает со значениями переменных, работает именно со стабильными переменными.
...
| Символ | Значение |
| ------ | -------- |
| `\[`   | Начало непечатаемых символов (цвет, жирность и прочее) |
| `\]`   | Конец непечатаемых символов |
