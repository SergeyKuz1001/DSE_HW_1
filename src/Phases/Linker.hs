...
--     потока ввода.
--
-- Таким образом из внутренних команд остаются @wc@ без аргументов, @pwd@ и
-- @grep@.
...
--
-- Заметим, что на входе в качестве чистой команды полагаются возможными только
-- команды @wc@ без аргументов и @grep@. На выходе же чистой командой также
